    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex gap-3">
                @if ($user->suspended)
                    <span
                        class="px-3 py-1 bg-red-100 text-red-600 rounded-full text-xs font-bold border border-red-200 flex items-center">
                        <i class="fas fa-ban mr-1.5"></i> Suspended
                    </span>
                @else
                    <span
                        class="px-3 py-1 bg-green-100 text-green-600 rounded-full text-xs font-bold border border-green-200 flex items-center">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="glass-panel p-6 text-center relative overflow-hidden group">
                    ...
                    <div class="relative z-10">
                        <div
                            class="w-24 h-24 mx-auto rounded-full bg-white shadow-lg p-1 mb-4 flex-shrink-0 overflow-hidden">
                            @if ($user->profile_photo)
                                <img src="{{ Storage::url($user->profile_photo) }}" alt="{{ $user->name }}"
                                    class="w-full h-full object-cover rounded-full">
                            @else
                                <div
                                    class="w-full h-full rounded-full bg-red-50 flex items-center justify-center text-2xl font-bold text-red-500">
                        ...
                        <div class="flex justify-center gap-2 mb-6">
                            <a href="mailto:{{ $user->email }}"
                                class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-red-600 transition-colors">
                        ...
                        <div class="border-t border-gray-100 pt-4 text-left">
                            <div class="flex items-center justify-between">
                                ...
                                <span
                                    class="text-sm font-bold text-gray-700">{{ $user->created_at->format('M d, Y') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        @if ($user->suspended)
                            <form action="{{ route('admin.users.suspend', $user->id) }}" method="POST">
                                ...
                                <button type="submit"
                                    class="w-full py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-green-200 transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
                                    <i class="fas fa-check"></i> Unsuspend User
                                </button>
                            </form>
                        @else
                            <button onclick="openSuspendModal()"
                                class="w-full py-2.5 bg-white border border-red-200 text-red-500 hover:bg-red-50 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="glass-panel p-6">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($user->preferred_categories as $category)
                                <span
                                    class="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-xs font-bold border border-red-100">
                                    {{ $category }}
                                </span>
                            @endforeach
                        </div>
                ...
                @if ($user->suspended && $user->suspension_reason)
                    <div class="glass-panel p-6 border-l-4 border-red-500">
                        ...
                        <p class="text-xs text-gray-400 mt-3">Suspended on
                            {{ \Carbon\Carbon::parse($user->suspended_at)->format('F d, Y') }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 mb-4">Temporarily disable this user's access. They will
                                        not be able to log in.</p>
                                    <form id="suspendForm" action="{{ route('admin.users.suspend', $user->id) }}"
                                        method="POST">
                                        @csrf
                                        <input type="hidden" name="type" value="suspend">
                                        <textarea name="suspension_reason" id="suspension_reason" rows="4"
                                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-orange-500 focus:border-orange-500"
                                            placeholder="Reason for suspension..." required></textarea>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <button type="submit" form="suspendForm"
                            class="inline-flex w-full justify-center rounded-xl bg-orange-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-orange-500 sm:ml-3 sm:w-auto">Suspend</button>
